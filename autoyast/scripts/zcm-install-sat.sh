...
# GNU Public License
#
# zcm-install-sat.sh	 					 9 Jan 2013
# last modified		 					28 Nov 2017

SCRIPT_NAME="zcm-install-sat.sh"
...
function config_satellite_script()
{
	SAT_SCRIPT=config-zcm-satellite.sh	
	get_file $AY_SERVER/scripts/$SAT_SCRIPT
	cp -p $PROFILE_DIR/$SAT_SCRIPT /var/adm/autoinstall/scripts/$SAT_SCRIPT

	# only configure when not already configured
	# This is only a workaround; real change needs to go into zmc.py 
	sh $SAT_SCRIPT
}


function distribute_satreinstall_file()
{
	SAT_REINST_SCRIPT=sat-reinstallation.sh
	$WGETCMD $AY_SERVER/scripts/$SAT_REINST_SCRIPT -O /var/adm/autoinstall/scripts/$SAT_REINST_SCRIPT
}


function exec_functions()
{
	local function_list=$@
	local my_function
	if [ -n "$function_list" ];then

		for my_function in $function_list;do
			 $my_function
		done
	fi
}
...
{
	# execute customer related functions if available, e.g. configure satellite servers
	grep -qi "is01\>" <<<"$my_hostname"

	if [ $? -eq 0 ]; then
		MY_FUNCTION_LIST="config_satellite_script distribute_satreinstall_file"
		exec_functions "$MY_FUNCTION_LIST"
	fi
}
